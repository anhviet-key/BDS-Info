...
export default function details() {
  ...
  useEffect(() => {
    ...
    //   setReport(res.data);
    // });
    JsonData.Datas.products.forEach((value, x) => {
      console.log(value);

      return value.id === productsId ? setReport(value) : '';
    });
  }, [productsId]);
  return (
    <div className="App">
      <div className="mb-10">
        ...
        <div className="mt-10 ml-10 grid grid-cols-2 gap-x-5 gap-y-10 ">
          <div className="contents mt-10">
            <div className="">
              <h1 className="lg:text-[42px] font-medium">{report.title}</h1>
              <p className="mt-5">{report.address}</p>
              <p className="mt-4">Diện tích: {report.area}m2</p>
            </div>
            <div className="text-[40px] lg:text-[100px] flex justify-center items-center font-semibold">
              <p>
          ...
          <div className="w-full">
            <h1 className="text-[38px] w-max font-medium mt-5">
              Thông tin chi tiết
            </h1>
            <p className="mt-5 w-fit">{report.description}</p>
          </div>
        </div>
      </div>
    </div>
